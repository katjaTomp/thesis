...
def loadFile(file):
    raw = mne.io.read_raw_edf(file, eog=['EXG1', 'EXG2', 'EXG3', 'EXG4', 'EXG5', 'EXG6'], stim_channel=-1,
                              preload=True)  # documentation here:https://martinos.org/mne/stable/generated/mne.io.read_raw_edf.html#mne.io.read_raw_edf
...
def genEpochs(raw, events, picks):
    event_id, tmin, tmax = [65281, 65284, 65286, 65288, 65294, 65296, 65298, 65304, 65306,
                            65308], 0, 0.5  # was=0.2, 0.5
...
epochs = downsampleEpochs(100, epochs)
...
index = ['time']  # 1, dict(grad=1e13)

df = epochs.to_data_frame(picks=None, scalings=None, index=index)
df.to_pickle('ppn1-downsampled.pkl')

print(df.dtypes)

fcz = df["FCz"]  # "condition","epoch","time","Fcz"

print(fcz)

numpfcz = np.array(fcz)
